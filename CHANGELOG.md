...
- Date synchronization uses one canonical remote task date: Logseq `DEADLINE` is synchronized, while `SCHEDULED` remains local-only planning metadata.
...
- Plugin lifecycle cleanup unregisters settings/database hooks and clears polling/debounce/suppression state on unload.
