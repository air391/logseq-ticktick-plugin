...
> **Graph compatibility:** the current supported target is a traditional Logseq **File Graph** (Markdown/Org). DB Graph support is not complete yet. The property-index layer is being made graph-aware, but task text/status/date synchronization still follows the File Graph model. Do not use this version for production synchronization in a DB Graph.
...
- Preserve edited or annotated `[[Dida Inbox]]` projections instead of silently deleting their local notes when the remote task is completed or linked elsewhere.
...
`[[Dida Inbox]]` projections follow the same conservative rule. New projections remember the exact content generated by the plugin. An untouched projection with no children can be removed automatically when it is no longer needed. If the projection was edited, has child blocks, or comes from an older version where the plugin cannot prove it is untouched, the plugin removes its projection metadata but preserves the Logseq content.
