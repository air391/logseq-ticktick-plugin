...
- `/Dida Pull Remote` — pull the linked remote task into the current block.
- `/Dida Pull All Linked` — pull all linked tasks.
...
Automatic synchronization will not silently perform ambiguous or destructive checklist operations such as:
...
Use an explicit conflict-resolution command when you really want one side to replace the other.
